...
class LitellmModel:
    ...
    def _query(self, messages: list[dict[str, str]], **kwargs):
        ...
        logger.debug(
            "Sending API request to model=%s, n_messages=%d, kwargs=%s",
            self.config.model_name,
            len(messages),
            {k: v for k, v in merged_kwargs.items() if k not in ("api_key",)},
        )
        try:
            ...
            logger.debug(
                "API response received in %.1fs, finish_reason=%s",
                time.time() - t0,
            ...
            logger.debug("API request failed after %.1fs: %s: %s", time.time() - t0, type(e).__name__, e)
            raise
    ...
    def query(self, messages: list[dict[str, str]], **kwargs) -> dict:
        logger.debug("query() called with %d messages", len(messages))
