...
def visualizer(image_path: str, question: str | None = None) -> str:
    ...
    api_base = (os.getenv("OPENAI_API_BASE") or os.getenv("LITELLM_API_BASE") or "https://api.openai.com/v1").rstrip("/")
    ...
    try:
        ...
        recorder = current_model_trace_recorder()
    except Exception:
        recorder = None
    try:
        ...
        raise Exception(f"Response format unexpected: {response_json}") from exc
