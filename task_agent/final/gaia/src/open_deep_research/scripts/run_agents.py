...
def get_single_file_description(file_path: str, question: str, visual_inspection_tool, document_inspection_tool):
    ...
    if file_extension in ["png", "jpg", "jpeg"]:
        ...
        try:
            description = get_image_description(file_path, question, visual_inspection_tool)
        except Exception as exc:
            description = f"Image description unavailable ({type(exc).__name__}: {exc})"
        ...
        if os.path.exists(image_path):
            ...
            try:
                ...
                description = f"Document description unavailable ({type(exc).__name__}: {exc})"
    ...
    for root, dirs, files in os.walk(folder_path):
        for file in files:
            ...
            try:
                ...
                description = f" - Attached file: {file_path}\n     -> File description unavailable ({type(exc).__name__}: {exc})"
