...
class XlsxConverter(HtmlConverter):
    ...
    def convert(self, local_path, **kwargs) -> None | DocumentConverterResult:
        ...
        engine = "xlrd" if extension.lower() == ".xls" else "openpyxl"
        try:
            sheets = pd.read_excel(local_path, sheet_name=None, engine=engine)
        except ImportError as exc:
            raise ImportError(
                f"Reading {extension.lower()} spreadsheets requires the optional "
                f"{engine!r} dependency."
            ) from exc
        md_content = ""
        for s in sheets:
            md_content += f"## {s}\n"
...
class Mp3Converter(WavConverter):
    ...
    def convert(self, local_path, **kwargs) -> None | DocumentConverterResult:
        ...
        os.close(handle)
        try:
            self._convert_to_wav(local_path, temp_path)
    ...
    def _convert_to_wav(self, local_path: str, temp_path: str) -> None:
        ...
        if ffmpeg:
            subprocess.run(
                [ffmpeg, "-y", "-i", local_path, "-ac", "1", "-ar", "16000", temp_path],
                check=True,
        ...
        if extension == ".mp3":
            ...
            sound = pydub.AudioSegment.from_file(local_path, format=extension.lstrip("."))
