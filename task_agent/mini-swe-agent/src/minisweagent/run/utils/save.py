...
def save_traj(
    ...
    **kwargs,
    ...
    if isinstance(messages, list) and "model_calls" not in data:
        model_calls = build_model_calls_from_messages(messages)
        data["model_calls"] = model_calls
        data["info"]["chat_trace"] = build_chat_trace_summary(messages, model_calls)
