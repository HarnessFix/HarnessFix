...
def _build_augmented_question(task: dict, model, document_inspection_tool) -> str:
...
def run_single_task(task: dict, model_id: str, output_dir: Path) -> dict:
    ...
    try:
        ...
        with model_trace_context(task_id) as trace_recorder:
            ...
            augmented_question = _build_augmented_question(task, file_context_model, document_inspection_tool)
...
def main() -> None:
    ...
    print(f"Running {len(tasks)} tasks | model={args.model} | workers={effective_workers}")
    ...
    def _run_one(task: dict) -> tuple[str, str, str]:
        try:
            result = run_single_task(task, args.model, args.output)
