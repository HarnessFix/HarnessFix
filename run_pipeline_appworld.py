#!/usr/bin/env python3
"""Closed-loop HarnessFix pipeline for AppWorld."""
...
APPWORLD_AGENT_DIR = TASK_AGENT_DIR / "appworld_agent"
TRAIN_DIR = DATA_DIR / "appworld_train_90"
VAL_DIR = DATA_DIR / "appworld_val_45"
TEST_DIR = DATA_DIR / "appworld_test_90"


def log(msg: str) -> None:
    print(f"\n[pipeline_appworld] {msg}", flush=True)
...
def base_agent_dir(version: int) -> Path:
    if version <= 0:
        return APPWORLD_AGENT_DIR
    return TASK_AGENT_DIR / f"enhanced_appworld_v{version}"


def base_agent_label(version: int) -> str:
    return "original" if version <= 0 else f"appworld_enhanced_v{version}"
...
def train_run_id(model: str, version: int) -> str:
    return f"appworld_train90_{model_slug(model)}_{base_agent_label(version)}"


def val_baseline_run_id(model: str) -> str:
    return f"appworld_val45_{model_slug(model)}_original"
...
def train_eval_dir(model: str, version: int) -> Path:
    return EVAL_DIR / f"appworld_results_{train_run_id(model, version)}"


def val_enhanced_run_id(model: str, version: int) -> str:
    return f"appworld_val45_{model_slug(model)}_enhanced_v{version}"
...
def val_enhanced_eval_dir(model: str, version: int) -> Path:
    return EVAL_DIR / f"appworld_results_{val_enhanced_run_id(model, version)}"
...
def val_eval_dir_for_base(model: str, base_version: int) -> Path:
    return EVAL_DIR / f"appworld_results_{val_baseline_run_id(model)}" if base_version <= 0 else val_enhanced_eval_dir(model, base_version)
...
def step_train_inference(model: str, base_version: int, workers: int, concurrency: int,
    ...
    if (output_dir / "preds.json").exists() and not force:
        ...
        return output_dir
    cmd = [
        "bash", str(TASK_AGENT_DIR / "run_appworld.sh"),
...
def step_train_evaluate(model: str, base_version: int, train_traces: Path,
    ...
    cmd = [
        VENV_PYTHON, EVAL_DIR / "eval_appworld.py",
        "--subset", str(TRAIN_DIR),
...
def step_train_failure_analysis(model: str, base_version: int, train_traces: Path, train_eval: Path,
    ...
    cmd = [
        ...
        "--appworld-subset", str(TRAIN_DIR),
        "--output-file", str(output_file),
        "--workers", str(workers),
        "--agent-source-dir", str(base_agent_dir(base_version) / "src" / "appworld_agent"),
        "--resume",
    ]
    if dry_run:
        print(f"  [DRY RUN] would run: {' '.join(str(c) for c in cmd)}")
...
def step_aggregate(version: int, model: str, analysis_results: Path, analysis_model: str,
                   ...
                   force: bool, dry_run: bool) -> Path:
    plan_path = IMPROVEMENT_PLANS_DIR / f"appworld_{model_slug(model)}_v{version}.md"
...
def _run_modify_versioned_appworld(version: int, base_version: int, plan_path: Path,
    ...
    shutil.copytree(source_dir, target_dir, ignore=shutil.ignore_patterns("__pycache__", "*.pyc", ".git", "*.egg-info"))

    config_path = REPO_ROOT / "enhancement_implementation" / "config_appworld.yaml"
    ...
    traj_path = REPO_ROOT / "enhancement_implementation" / "results" / f"modify_appworld_v{version}.traj.json"
    ...
    agent = DefaultAgent(model, env, output_path=traj_path, **{k: v for k, v in agent_config.items() if k not in ("system_template", "instance_template")}, system_template=agent_config["system_template"], instance_template=agent_config["instance_template"])
    agent.run(target_dir=str(target_dir), original_dir=str(source_dir), plan_path=str(plan_path), plan_json_path=str(spec_path))
...
def step_modify(version: int, base_version: int, plan_path: Path, modify_model: str,
    ...
    target_dir = TASK_AGENT_DIR / f"enhanced_appworld_v{version}"
    ...
    if dry_run:
        ...
        return target_dir
    _run_modify_versioned_appworld(version, base_version, plan_path, plan_spec_path(plan_path), target_dir, modify_model, train_compare_path)
    return target_dir


def step_plan_diff_audit(version: int, base_version: int, plan_path: Path, target_dir: Path, dry_run: bool) -> dict:
    output_path = FAILURE_ANALYSIS_DIR / "results" / f"audit_appworld_v{version}.json"
...
def step_val_baseline_inference(model: str, workers: int, concurrency: int, force: bool, dry_run: bool) -> Path:
    output_dir = TRACES_DIR / val_baseline_run_id(model)
    log(f"Bootstrap: val baseline inference → {output_dir.name}/")
    if (output_dir / "preds.json").exists() and not force:
        log("  val baseline preds already exist, skipping")
        return output_dir
    cmd = [
        "bash", str(TASK_AGENT_DIR / "run_appworld.sh"),
        "--subset", str(VAL_DIR),
...
def step_val_inference(version: int, enhanced_src: Path, model: str, workers: int, concurrency: int,
                       dry_run: bool) -> Path:
...
def step_val_evaluate(version: int, val_traces: Path, model: str, dry_run: bool) -> Path:
    ...
    if find_eval_json(report_dir):
        log("  eval results already exist, skipping")
        return report_dir
    output_file = report_dir / "results.json"
    cmd = [
        VENV_PYTHON, EVAL_DIR / "eval_appworld.py",
...
def step_compare_train(version: int, model: str, baseline_train_eval: Path,
    ...
    output = FAILURE_ANALYSIS_DIR / "results" / f"appworld_train_compare_v{version}.json"
    log(f"Step 6.5: compare train baseline vs enhanced v{version}")
    if dry_run:
        return {"net_change": 0, "dry_run": True, "regressed_ids": [], "improved_ids": [], "baseline_count": 0, "current_count": 0, "regression_count": 0, "improvement_count": 0}
    cmd = [
    ...
    return {"net_change": 0, "regressed_ids": [], "improved_ids": [], "baseline_count": 0, "current_count": 0, "regression_count": 0, "improvement_count": 0}
...
def step_gate_check(version: int, baseline_eval: Path, val_eval: Path, baseline_traces: Path,
                    val_traces: Path, plan_path: Path, max_regression: int, min_improvement: int,
                    max_cost_ratio: float, dry_run: bool) -> dict:
    output_path = FAILURE_ANALYSIS_DIR / "results" / f"gate_appworld_v{version}.json"
    ...
    cmd = [
        ...
        "--current-eval", str(val_eval),
        "--baseline-traces", str(baseline_traces),
        "--current-traces", str(val_traces),
        "--plan-spec", str(plan_spec_path(plan_path)),
...
def step_analyze_regressions(version: int, regressed_ids: list[str], val_traces: Path,
    ...
    output_file = FAILURE_ANALYSIS_DIR / "results" / f"appworld_val_regression_analyses_v{version}.jsonl"
    ...
    try:
        cmd = [
            ...
            "--model", analysis_model,
            "--traces-dir", str(val_traces),
            "--appworld-subset", str(VAL_DIR),
            "--output-file", str(output_file),
            "--instance-ids-file", str(ids_file),
            "--workers", str(workers),
            "--agent-source-dir", str(base_agent_dir(version) / "src" / "appworld_agent"),
            "--no-resume",
        ]
        run(cmd)
    finally:
...
def print_test_eval_command(enhanced_dir: Path, model: str) -> None:
    ...
    print(f"""
Next: run final AppWorld held-out test evaluation with {version}

  APPWORLD_ROOT=/path/to/appworld_root \\
  APPWORLD_AGENT_IMAGE=appworld-agent-pypi:latest \\
  AGENT_SRC_DIR={enhanced_dir}/src \\
  ./task_agent/run_appworld.sh \\
      --subset "$(pwd)/data/appworld_test_90" \\
      --model {model} \\
      --workers 2 --concurrency 4 \\
      --output traces/appworld_test_{model_slug(model)}_{version}

  mkdir -p eval/appworld_results_test_{version}
  .venv/bin/python3 eval/eval_appworld.py \\
      --subset data/appworld_test_90 \\
      --traces-dir traces/appworld_test_{model_slug(model)}_{version} \\
      --output eval/appworld_results_test_{version}/results.json
""")
...
def main() -> None:
    ...
    if not TRAIN_DIR.exists():
        print("ERROR: data/appworld_train_90/ not found. Run: python3 data/sample_appworld.py")
        raise SystemExit(1)
    if not VAL_DIR.exists():
        print("ERROR: data/appworld_val_45/ not found. Run: python3 data/sample_appworld.py")
        raise SystemExit(1)
    if not TEST_DIR.exists():
        print("ERROR: data/appworld_test_90/ not found. Run: python3 data/sample_appworld.py")
        raise SystemExit(1)

    print(f"""
{'='*60}
  HarnessFix Pipeline (AppWorld)
  model:            {args.model}
...
""")
    ...
    baseline_traces = step_val_baseline_inference(args.model, args.workers, args.concurrency, args.force, args.dry_run)
    ...
    for i in range(args.max_iterations):
        ...
        print(f"\n{'#'*60}")
        print(f"  APPWORLD ITERATION {i+1}/{args.max_iterations}  (candidate v{version}, base v{current_base_version})")
        print(f"{'#'*60}")

        train_traces = step_train_inference(args.model, current_base_version, args.workers, args.concurrency, args.force, args.dry_run)
        ...
        for retry in range(args.max_modify_retries + 1):
            ...
            enhanced_train_traces = step_train_inference(args.model, version, args.workers, args.concurrency, (retry > 0) or args.force, args.dry_run)
            enhanced_train_eval = step_train_evaluate(args.model, version, enhanced_train_traces, (retry > 0) or args.force, args.dry_run)
            train_compare = step_compare_train(version, args.model, train_eval, enhanced_train_eval, train_traces, enhanced_train_traces, plan_path, args.dry_run)
            train_net = train_compare.get("net_change", 0)
            train_improved = train_net > 0
            log(f"  Train compare (v{version} retry={retry}): net={train_net:+d} improvements={train_compare.get('improvement_count', 0)} regressions={train_compare.get('regression_count', 0)}")
            if train_improved or args.dry_run:
                break
            if retry < args.max_modify_retries:
                train_compare_path = FAILURE_ANALYSIS_DIR / "results" / f"appworld_train_compare_v{version}.json"
                if not args.dry_run:
                    _cleanup_for_redo(version, args.model)
            else:
        ...
        val_traces = step_val_inference(version, target_dir / "src", args.model, args.workers, args.concurrency, args.dry_run)
        ...
        gate = step_gate_check(version, base_val_eval, val_eval, base_val_traces, val_traces, plan_path, args.max_regression, args.min_improvement, args.max_cost_ratio, args.dry_run)
        step_record_memory(version, plan_path, audit, gate, args.dry_run)
        if gate.get("passed"):
            ...
            log(f"✗ NOT PROMOTED v{version}: {gate.get('regression_count', '?')} regressions, {gate.get('improvement_count', '?')} improvements")
